...
export class BackupService {
  private static readonly BACKUP_DIR =
    process.env.BACKUP_PATH || path.join(__dirname, "../../data/backups");
  private static readonly DB_PATH =
    process.env.DB_PATH || path.join(__dirname, "../../data/flights.db");
  ...
  static async createBackup() {
    // Skip backup if database file does not exist (e.g., using Postgres instead of SQLite)
    if (!fs.existsSync(this.DB_PATH)) {
      console.warn(
        `Backup skipped: database file not found at ${this.DB_PATH}`
      );
      return;
    }
    try {
